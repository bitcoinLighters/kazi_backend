export const swaggerSpec = {
  ...
  paths: {
    ...
    '/api/tasks/{id}/submissions': { post: { tags: ['Submissions'], summary: 'Submit completed work and worker invoice (youth)', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/id' }], requestBody: { required: true, content: { 'application/json': { schema: { type: 'object', required: ['description', 'invoice'], properties: { description: { type: 'string' }, fileUrl: { type: 'string' }, invoice: { type: 'string', description: 'Worker-owned BOLT11 invoice; client never supplies this' } } } } } }, responses: { 201: { description: 'Submission recorded' }, 400: { description: 'Missing or invalid invoice' } } } },
    ...
    '/api/tasks/{id}/approve-payment': { post: { tags: ['Payments'], summary: 'Client approves and pays worker invoice', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/id' }], responses: { 200: { description: 'Payment confirmed' }, 400: { description: 'Invoice amount invalid' }, 409: { description: 'Task not payable or payment processing' }, 502: { description: 'Payment failed; task remains reviewing' } } } },
    '/api/payments/{id}': { get: { tags: ['Payments'], summary: 'Get payment status', security: [{ bearerAuth: [] }], parameters: [{ $ref: '#/components/parameters/id' }], responses: { 200: { description: 'Payment status' }, 404: { description: 'Not found' } } } }
  }
};
